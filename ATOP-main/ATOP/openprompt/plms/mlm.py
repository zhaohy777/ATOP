...
class MLMTokenizerWrapper(TokenizerWrapper):
    ...
    def tokenize_one_example(self, wrapped_example, teacher_forcing):
        ...
        for piece in wrapped_example:
            ...
            if piece['soft_token_ids'] >= 1:
                encoder_inputs['input_ids'].append([encode_text[-1]])
            else:
                encoder_inputs['input_ids'].append(encode_text)  # <soft>x的编码(4个)
            for key in piece:
                if key != 'text' and piece['soft_token_ids'] == 0:
                    encoder_inputs[key].append([piece[key]] * encoding_length)  # 填充了4个soft_token_ids
                if key != 'text' and piece['soft_token_ids'] >= 1:
