    ...
    def train_epoch(self, epoch):
        ...
        for step, batches in enumerate(pbar):
            ...
            if self.config.dataset.adv:
                # gamma = 2 / (1 + math.exp(-10 * (now_iter) / 20*328)) - 1 
                ...
                gamma2 = gamma2.cuda()
                loss_domain = torch.mean(self.domain_adv(hidden_list, gamma2.unsqueeze(-1)))
            ...
            if NA:
                ...
                loss_ = torch.nn.CrossEntropyLoss(reduction='none')(logits_target, pred) 
