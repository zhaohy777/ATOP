...
def save_checkpoint(state_dict, is_best, save_path, filename='checkpoint.pt', domain=None):
    ...
    if is_best:
        full_best_path= os.path.join(save_path, 'best'+str(domain)+'.'+filename)
        logger.info("Saving the best checkpoint.")
        shutil.copyfile(full_file_path, full_best_path)
            
def load_checkpoint(load_path, load_best, filename="checkpoint.pt", map_location=None,domain=None):
    r"""load the checkpoint from :obj:`load_path`.
    """
    if load_best:
        full_file_path= os.path.join(load_path, 'best'+str(domain)+'.'+filename)
